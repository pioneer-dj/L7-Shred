...
type Config struct {
    ...
    ProxyFlow      string `json:"proxy_flow"`
    ProxyNetwork   string `json:"proxy_network"`
    ProxyPath      string `json:"proxy_path"`
    ProxySecurity  string `json:"proxy_security"`
    ProxyEncryption string `json:"proxy_encryption"`
...
func LoadConfig(path string) (*Config, error) {
    ...
    if config.ProxyPort == 0 && config.ProxyEnabled {
        config.ProxyPort = 8446
    }
    if config.ProxyType == "" && config.ProxyEnabled {
        config.ProxyType = "socks5"
    }
    if config.ProxyHost == "" && config.ProxyEnabled {
